...
$$
\mathcal{E}(\lambda, \Gamma) := \left\{f \colon M \to M :|f|_{c^{2}} \leq \Gamma,\quad |Df(x) v| \geq  \lambda|v| \ \forall(x, v)\in TM \right\}.
$$
...
$$
\mathcal{D} := \left\{\varphi>0 : \int \varphi dm =1, \quad \varphi \text { is Lipschitz }\right\}.
$$
