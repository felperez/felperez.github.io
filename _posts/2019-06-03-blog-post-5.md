...
**Theorem:**
If $\psi\in L^\infty$, we have decay of correlations with rate $r(n)$.

**Proof:** observe that
$$
\left| \int_X \phi\cdot \psi\circ T^n d\mu \right| = \left| \int_X P^n\phi \cdot \psi d\mu\right| \leq |P^n\phi |_{L^1} | \psi |_{L^\infty}\leq r(n)|\psi |_{L^\infty} | \psi |_{\mathcal{B}_\alpha}.\square
$$
...
This semi-norm can easily be turned into a norm by adding the $\mathcal{C}^0$ norm to it, but for the estimates this is not relevant. Then for $\phi$ with $\int\_X \phi dm =0$, one has $\|\phi \|\_{L^1} \leq  \|\phi\|\_{Lip}$. The Lipschitz semi-norm of the action of the transfer operator can be bounded by
