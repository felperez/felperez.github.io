...
This semi-norm can easily be turned into a norm by adding the $\mathcal{C}^0$ norm to it, but for the estimates this is not relevant. Then for $\phi$ with $\int\_X \phi dm =0$, one has $\|\phi \|\_{L^1} \leq  \|\phi\|\_{Lip}$: since $\int\_X \phi dm =0$, there is a point $x_0$ such that $\phi(x_0) = 0$. This implies that
