...
Suppose that the transfer operator $P:\mathcal{B}\to\mathcal{B}$ has spectral gap for some Banach space of functions $\mathcal{B}\subset L^1$. Recall the correlation function for observables $\phi,\psi\in\mathcal{B}$:

$$
C_{\phi,\psi}(n) = \int_X \phi\cdot \psi\circ T^n dm - \int_X \phi dm \int_X\psi dm.
$$

We can assume wlog that the observables are centered. Write the decomposition of the Banach space $\mathcal{B} = F\oplus H$ as above, so for $\phi\in\mathcal{B}$, there are $\phi_F\in F$ and $\phi_H\in H$ such that $\phi = \phi_F + \phi_H$. Then,

$$
C_{\phi,\psi}(n) = \int_X \phi\cdot \psi\circ T^n dm = \int_X P^k\phi \cdot \psi dm.
$$
