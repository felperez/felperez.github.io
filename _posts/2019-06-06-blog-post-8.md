...
Write the decomposition of the Banach space $\mathcal{B} = F\oplus H$ as above, so for $\phi\in\mathcal{B}$, there are $\phi_F\in F$ and $\phi_H\in H$ such that $\phi = \phi_F + \phi_H$. Then,

$$
C_{\phi,\psi}(n) = \int_X \phi\cdot \psi\circ T^n dm - \int_X \phi dm \int_X\psi dm = \int_X P^k\phi \cdot \psi dm - \int_X \phi dm \int\psi dm.
$$
