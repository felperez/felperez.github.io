...
Write the decomposition of the Banach space $\mathcal{B} = F\oplus H$ as above. Since $\dim(F) = 1$, we can pick a basis of $F$ consisting of an eigenfunction of $L$, that is, there is $f\in F$ and $c_F\in\mathbb{C}$ such that $L f = c\_F f$. For $\phi\in\mathcal{B}$, there are $c\_\phi\in\mathbb{C}$ and $\phi_H\in H$ such that $\phi = c\_\phi f + \phi_H$. Then,

$$
\int_X \phi\cdot \psi\circ T^n dm = \int_X P^k(c\_\phi f + \phi_H) \cdot \psi dm 
$$
