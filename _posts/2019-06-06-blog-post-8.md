...
**Decay of correlations:** Let $\psi\in L^\infty$ and $\phi\in\mathcal{B}$. We will suppose that $\mathcal{B}$ is closed under products. The correlation function with respect to $\mu$ for observables $\phi,\psi\in\mathcal{B}$ is given by:

$$
C_{\phi,\psi}^\mu(n) = \int_X \phi\cdot \psi\circ T^n fdm - \int_X \phi fdm \int_X\psi fdm.
$$

Define the operator $\Pi\colon L^1\to L^1$ by $\Pi(\phi) = m(\phi) f$, and the complementary operator $N\colon L^1\to L^1$ by $N(\phi) = P(\phi) - \Pi(\phi)$. Note that $\Pi$ and $N$ commute, so $P^n = \Pi^n + N^n$. Then we can write the correlation function as

$$
C_{\phi,\psi}^\mu(n) = \int_X P^n(\phi f)\psi dm -  \int_X\psi f \Pi(\phi f) dm\right) fdm = \int_X N^n(\phi f) \psi f dm
$$


$$
\int_X \phi\cdot \psi\circ T^n d\mu = \int_X P^k(c_\phi f + \phi_H) \cdot \psi dm = \int_X c_\phi c_F^k f \psi dm + \int_X P^k (\phi_H)  \psi dm.
$$
