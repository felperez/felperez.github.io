...
Note that this semi-norm vanishes for all constant functions. We can either mod out $\mathcal{B}$, or modify the definition of the norm, adding a contributuion of the supremum norm. We chose the later, and $\\| \cdot\\|\_{Lip}$ as

$$
|\phi|_{Lip} = \sup_{x\neq y}\dfrac{|\phi(x)-\phi(y)|}{|x-y|} + |\phi|_{L^\infty}.
$$
...
, and its associated projection $\Pi\colon \mathcal{B}\to F$ by $\Pi(\phi) = m(\phi)\mathbf{1}$
