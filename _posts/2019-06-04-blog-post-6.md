...
We can now give a definition of a martingale. A sequence of sub sigma-algebras $(\mathcal{F_n})$ is a *filtration* if $\mathcal{F}_n\subset\mathcal{F}_{n+1}$ for all $n$. A stochastic process $(M_n)$ is a *martingale* with respect to the filtration $(\mathcal{F}_n)$ if
