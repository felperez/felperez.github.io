...
**Theorem:** Suppose $\phi \in L^2$ and $\phi\_n = \phi\circ T^n$ for $T$ ergodic. Let $\mathcal{G}_j$ a filtration so that $\phi_n$ is adapted to it and $\mathbb{E}(\phi_{n} \| \mathcal{G}_{n+1})=0$ almost surely. Then it is possible to find centered Gaussian random variables $(Z_j)$ with $\mathbb{E}(Z_{k}^{2})=\mathbb{E}(\phi_{1}^{2})$ such that 

$$
\sup _{1 \leq k \leq n}\left|\sum_{i=1}^{k} \phi_{i}-\sum_{i=1}^{k} Z_{i}\right|=o(\sqrt{n \log \log n})
$$
