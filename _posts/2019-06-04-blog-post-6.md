...
$$
\int \phi\cdot\psi\circ T d\mu = \int P\phi \cdot \psi d\mu
$$
...
1. $\mathbb{E}(\phi \| T^{-1}\mathcal{B}) = UP\phi = (P\phi)\circ T$ and similarly $\mathbb{E}(\phi \| T^{-i}\mathcal{B}) = U^iP^i\phi = (P\phi)\circ T$.
...
for every set $A\in T^{-1}\mathcal{B}$.
