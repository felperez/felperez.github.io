...
$$
10^m a_m \leq 2^ n <10^{m} (a_m+1),
$$

and this is equivalent to

$$
m+ \log a_m \leq n\log 2 < m + \log(a_m+1).
$$

If we work$\pmod {1}$ then the above inequality is equivalent to

$$
\log a_m \leq n\log 2 \pmod {1} = T^n(0) <  \log(a_m+1).
$$
...
$$
f_{a_m} = \lim_{n\to\infty} \dfrac{1}{n}\text{\#of times }a_m\text{ is the leading digit of }2^k, k=1\dots n = \int_0^1 \chi_{[\log a_m, \log(a_m+1))}dx = \log(1+\dfrac{1}{a_m})
$$
...
https://web.williams.edu/Mathematics/sjmiller/public_html/BrownClasses/197/benford/Benford_AVKontorovichSJMIller_Final06b_aa.pdf
